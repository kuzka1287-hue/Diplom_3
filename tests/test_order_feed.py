...
class TestOrderFeed:
    ...
    def test_total_orders_counter_increases(self, logged_in_driver):
        ...
        # Переходим в ленту заказов и запоминаем начальное значение счётчика
        ...
        token_cookie = driver.get_cookie("accessToken")
        ...
        # Обновляем страницу и проверяем, что счётчик увеличился
        driver.refresh()
        main_page.go_to_orders_feed()
    ...
    def test_order_appears_in_work_section(self, logged_in_driver):
        ...
        # Переходим в ленту заказов (чтобы потом проверить появление)
        main_page.go_to_orders_feed()
        feed_page = OrdersFeedPage(driver)
        ...
        # Проверяем, что номер заказа присутствует в списке "В работе"
        assert str(order_number) in orders_in_work, \
            f"Номер заказа {order_number} не появился в 'В работе'. Текущие заказы: {orders_in_work}"
